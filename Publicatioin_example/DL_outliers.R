#### These are scripts for analysis of simulations using three approaches, PCAdapt, RDA, and DeepScan
### 
impdir=list.dirs(path = "~/Simulations/Selection/IMP_DL_Scan", full.names = TRUE, recursive = TRUE)
impfile <- list.files(impdir, pattern="*_mlpKerasDropout_env.csv", full.names = TRUE)
...
impfile[11:20]## each 10 file is a simulation
View(impSim_all[[20]]) ### x is the loci ID and Overall is the overall importance (RMSE or MAE), 1000 list: including 100 simulations and 10 envir

### combine every factor (envir) into one file, meaning combining each single simulation into one file

impSim_all[[11:20]]### This is sim1, check the list file
...
{
  loadings<-DL_data[,1:as.numeric(nenvir)]
  resscale <- apply(loadings, 2, scale)
  resmaha <- covRob(resscale, distance = TRUE, na.action= na.omit, estim="mcd")$dist
...
## Manhattan plot
ggplot() +
  geom_point(aes(x=which(Loci=="Neutral"), y=-log10(Simqvalue[-which(Loci!="Neutral"),1])), col = "gray83") +
  geom_point(aes(x=which(Loci!="Neutral"), y=-log10(Simqvalue[-which(Loci=="Neutral"),1]), colour = Selected_Loci)) +
  xlab("SNPs") + ylab("DL with condition)") +
  theme_bw()
