...
DLsim1=DLdata[1:10] ## This is the simulation 1, check
...
Simqvalue=DLqvalueslog(DLsim1,10)


## Manhattan plot
ggplot() +
  geom_point(aes(x=which(Loci=="Neutral"), y=-log10(Simqvalue[-which(Loci!="Neutral"),3])), col = "gray83") +
  geom_point(aes(x=which(Loci!="Neutral"), y=-log10(Simqvalue[-which(Loci=="Neutral"),3]), colour = Selected_Loci)) +
  xlab("SNPs") + ylab("DL with condition)") +ylim(c(0,150))+theme_bw()
...
p.values_DLmlp_simuslog <- lapply(DLdatasim, function(x) {DLqvalueslog(x, 10)[,1]})
pvalues_DLmlp_simus <- lapply(DLdatasim, function(x) {DLqvalues(x, 10)[,1]})
