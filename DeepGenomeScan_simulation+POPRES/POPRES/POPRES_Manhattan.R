...
POPRES_KLFDAPC_pvalue=DLqvalues(POPRES_IMP_KLFDAPC_RD1_RD2[,-1],2)
...
library(qqman)
Loci_under_selection=POPRES_ManHattan_data[(which(POPRES_ManHattan_data$P<1e-100)),]
...
manhattan(POPRES_ManHattan_data_don,ylim=c(0, 400),ylab=expression(-log[10](italic("q")-value)),annotatePval = 1e-100,highlight=Annotated_rs_genes$rsID,annotateTop = FALSE,suggestiveline = FALSE,genomewideline = -log10(1e-100))
dev.off()
...
manhattan(POPRES_ManHattan_data_don,ylim=c(0, 300),ylab=expression(-log[10](italic("q")-value)),annotatePval = 1e-100,highlight=Annotated_rs_genes$rsID,annotateTop = FALSE,suggestiveline = FALSE,genomewideline = -log10(1e-100))
dev.off()
